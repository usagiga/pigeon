...
func TestImageStoreKeeperUseCaseImpl_Store(t *testing.T) {
	...
	imageInfra.EXPECT().Fetch(gomock.Any(), "https://example.com/test.png").Return(nil).Times(2)
	imageInfra.EXPECT().Fetch(gomock.Any(), "https://example.com/error.png").Return(errors.New("error")).Times(1)
